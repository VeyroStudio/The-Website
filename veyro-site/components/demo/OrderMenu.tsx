    ...
    <div className="pb-28">
      {menu.map((section) => (
        <section key={section.category} className="mt-12 first:mt-0" data-reveal>
          ...
          <ul className="stagger mt-6 divide-y divide-white/10 border-y border-white/10">
            {section.items.map((item) => (
              <li key={item.name} className="flex items-center gap-4 py-4">
                ...
                <button
                  ...
                  className="inline-flex size-11 shrink-0 cursor-pointer items-center justify-center rounded-full border-2 border-white/20 text-xl font-bold transition-colors hover:border-[var(--accent)] hover:text-[var(--accent)]"
                >
                  +
                </button>
              </li>
      ...
      {/* Sticky basket bar */}
      {count > 0 && (
        <div className="demo-basket-in fixed inset-x-0 bottom-0 z-40 border-t border-white/10 bg-[#181a1f]/95 backdrop-blur-md">
          ...
          {/* Bottom sheet */}
          {open && (
            <div className="mx-auto max-w-3xl border-t border-white/10 px-4 py-4">
              {placed ? (
                ...
                <>
                      ...
                      <li key={l.name} className="flex items-center gap-3">
                        ...
                        <div className="flex items-center gap-2">
                          <button
                            ...
                            className="inline-flex size-9 cursor-pointer items-center justify-center rounded-full border border-white/20 transition-colors hover:border-[var(--accent)]"
                          >
                            −
                          </button>
                  ...
                  <button
                    ...
                    className="mt-4 inline-flex min-h-12 w-full cursor-pointer items-center justify-center rounded-full text-base font-bold text-black transition-opacity hover:opacity-85"
                    style={{ background: accent }}
                  >
                    Checkout — {gbp(total)}
                  </button>
